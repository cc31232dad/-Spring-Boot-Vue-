...
<template>
  <main class="home-page">
    <header class="home-header">
      ...
      <div class="account-actions"><RouterLink class="secondary-link" :to="{ name: 'seckill' }">秒杀专区</RouterLink><RouterLink class="secondary-link" :to="{ name: 'cart' }">购物车</RouterLink><template v-if="auth.isLoggedIn"><details class="user-dropdown"><summary><span class="avatar-placeholder small">{{ (auth.username || '农').slice(0, 1).toUpperCase() }}</span>{{ auth.username || '农友' }}</summary><div class="dropdown-menu"><RouterLink :to="{name:'user'}">个人中心</RouterLink><RouterLink v-if="auth.roles.includes('USER')" :to="{name:'user-orders'}">我的订单</RouterLink><RouterLink v-if="auth.roles.includes('USER')" :to="{name:'user-address'}">收货地址</RouterLink><RouterLink v-if="auth.roles.includes('USER')" :to="{name:'user-favorites'}">收藏商品</RouterLink><RouterLink v-if="auth.roles.includes('USER')" :to="{name:'user-settings'}">账户设置</RouterLink><RouterLink v-if="auth.roles.includes('FARMER')" :to="{name:'farmer-products'}">卖家中心</RouterLink><RouterLink v-if="auth.roles.includes('FARMER')" :to="{name:'farmer-products'}">商品管理</RouterLink><RouterLink v-if="auth.roles.includes('ADMIN')" :to="{name:'admin-products'}">商品审核</RouterLink><RouterLink v-if="auth.roles.includes('ADMIN')" :to="{name:'admin-farmers'}">农户审核</RouterLink><button type="button" @click="logout">退出登录</button></div></details></template><RouterLink v-else class="secondary-link" :to="{ name: 'login' }">登录</RouterLink></div>
    </header>
