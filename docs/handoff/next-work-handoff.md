...
P0-1 已完成：Flyway 已迁移到 V6，商品新增 `PENDING_REVIEW`、`REJECTED` 状态及审核字段；农户创建、修改和重新上架都会进入待审核，公开接口仍只返回 `ON_SALE`。P0-2 后端审核 API 已完成。P0-3/P0-4 前端农户商品状态页和管理员审核页也已接入，并新增农户本人商品列表接口。完整后端 `mvn test` 通过 77 项，前端通过 25 项测试及生产构建。
...
已新增订单详情页、物流信息占位和再次购买流程；支付边界、评价入口仍待完善。
...
前端：进入 `frontend` 执行 `npm run test -- --run` 和 `npm run build`；2026-08-19 已通过 25 项，构建成功。
